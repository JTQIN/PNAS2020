  ...
  # begin initialise.parameters
{
  ...
  if (is.null(PL$K_O2))   PL$K_O2    <- 0.002           # Monod constant O2 consumption [umol cm-3 or mM] (Meysman et al., 2003)
  ...
  if (is.null(PL$k.IOA)) PL$k.IOA <- 0.6  # kinetic constant iron oxide ageing [yr-1] (Berg et al., 2003)
  
  #if (is.null(PL$k.FIS)) PL$k.FIS <- 1    # kinetic constant Ferrous iron sorption  [yr-1] (Berg et al., 2003)
  if (is.null(PL$K.FIS)) PL$K.FIS <- 0.1*PL$rho.sed*268   # Equilibrium constant Ferrous iron sorption [] (Berg et al., 2003)
  ...
  if (is.null(PL$a56Fe_FIS))     PL$a56Fe_FIS     <- (1.0 + 0.0/1000.0) # alpha for Ferrous Iron Sorption
  ...
  if (is.null(PL$HCO3.ow))     PL$HCO3.ow    <- 2.45 # SumCO2 concentration bottom water [umol cm-3 or mM]
