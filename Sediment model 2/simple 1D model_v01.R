...
#     SR.s$R <- SO4.lim*O2.inh*Cmin.s$R # Sulfate reduction
    ...
    SIR$R  <- svf.grid$mid*k.SIR*FeOOH$C*FSAT(FeOOH$C,C.lim/100,5)*HS$C*FSAT(HS$C,C.lim/100,5)  # Sulfide re-oxidation by FeIII
    ...
    OC$kin.reac    <- - (AR$R + DIR$R + SR$R)/svf.grid$mid
    FeOOH$kin.reac <- (- 4*DIR$R + 4*FIO$R + ISO$R - 8*SIR$R)/svf.grid$mid
    FeS$kin.reac   <- (ISP$R - ISO$R)/svf.grid$mid
        
    O2$kin.reac  <- (- AR$R - FIO$R - 2*CSO$R - 9/4*ISO$R)/por.grid$mid
    Fe$kin.reac  <- (4*DIR$R - 4*FIO$R  + 8*SIR$R - ISP$R)/por.grid$mid
    SO4$kin.reac <- (- 0.5*SR$R + SIR$R + CSO$R + ISO$R)/por.grid$mid
    HS$kin.reac  <- (0.5*SR$R - SIR$R - CSO$R - ISP$R)/por.grid$mid
    HCO3$kin.reac<- (AR$R + DIR$R + SR$R)/por.grid$mid
    H2O$kin.reac <- (6*DIR$R - 6*FIO$R - 3/2*ISO$R + 12*SIR$R)/por.grid$mid
    H$kin.reac   <- (AR$R - 7*DIR$R + 1/2*SR$R + 8*FIO$R + 2*ISO$R - 15*SIR$R + CSO$R + ISP$R)/por.grid$mid
